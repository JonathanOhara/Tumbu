...
//-------------------------------------------------------------------------------------
SpecialPunch::~SpecialPunch(void){
	clear();
...
//-------------------------------------------------------------------------------------
void SpecialPunch::update(const Ogre::Real time){
...
//-------------------------------------------------------------------------------------
void SpecialPunch::collision( CollisionDetectionListener *other ){
	if( getSpecialStatus() == SpecialInterface::ATTACKING ){
		...
		while ( j != robotSpeller->enemyList.end() ){
			...
			if( enemy->charRigidBody->getBulletRigidBody() == otherRigidBody ){
				enemy->emmitSound( "specialPunchSound", "punch", "punch.ogg", false, true, 0.0f);
				hit( enemy->robotNode );
...
void SpecialPunch::concentrate(){
	...
	Ogre::Vector3 position(	robotSpeller->rightArmNode->_getDerivedPosition() );
...
void SpecialPunch::attack(Ogre::Quaternion orientation){
	...
	Ogre::Vector3 translation = orientation * Ogre::Vector3(0, 0, 250);
...
//-------------------------------------------------------------------------------------
void SpecialPunch::hit(Ogre::SceneNode* hittedNode){
	setSpecialStatus( SpecialInterface::HITTED );
}
//-------------------------------------------------------------------------------------
void SpecialPunch::hitScenario( Ogre::Vector3 position ){
	setSpecialStatus( SpecialInterface::HITTED );
}
//-------------------------------------------------------------------------------------
void SpecialPunch::toDelete(){
	setSpecialStatus( SpecialInterface::TO_DELETE );
}
//-------------------------------------------------------------------------------------
